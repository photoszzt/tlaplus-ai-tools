...
sany_parse --file MySpecAnim.tla

# Explore with animation
tlc_explore --spec MySpecAnim --config MySpec.cfg
```
...
- `sany_parse`: Validate animation spec syntax
- `tlc_explore`: Generate and view animation
- `tlc_check`: Run model checker with animation output
...
1. **Detect**: Call `animation_detect` to query the terminal environment. This inspects environment variables (`TERM`, `TERM_PROGRAM`, `KITTY_WINDOW_ID`, `TMUX`, etc.) and returns a structured result describing your terminal capabilities.
...
3. **Render**: Call `animation_render` with the chosen protocol and your animation data (AnimView record, SVG string, or SVG file path). The tool returns the rendered frame for display.

4. **Navigate**: For trace visualizations with multiple frames, call `animation_frameCount` first to discover available frames, then render each frame individually by its file path.

---

## Detection Result Interpretation

When you call `animation_detect`, it returns a JSON object with the following fields:
...
For trace visualizations (counterexample traces, behavior exploration), animations consist of multiple frames stored as SVG files in a directory. Use the `animation_frameCount` tool to discover and navigate these frames.

### Step 1: Discover Available Frames

Call `animation_frameCount` with the trace directory:
...
Use `animation_render` with the specific file path from the list:
...
### Tool Parameters: `animation_frameCount`
