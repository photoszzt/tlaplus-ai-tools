...
if (!model) {
  log("SKIP: OPENCODE_MODEL not set", "yellow");
  log(
    'To run E2E tests: OPENCODE_E2E=1 OPENCODE_MODEL="<model>" npm run opencode:e2e',
    "gray",
  );
  process.exit(0);
}
...
async function runCommand(command, args, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timeoutId = setTimeout(() => {
      proc.kill("SIGTERM");
      reject(
        new Error(`Command timed out after ${options.timeout || 120000}ms`),
      );
...
async function preflight() {
  ...
  try {
    const { code, stdout, stderr } = await runCommand("opencode", [
      "--version",
    ]);
    if (code !== 0) {
  ...
  try {
    ...
    if (code !== 0) {
      ...
      if (
        ...
        log(
          "\nDiagnostic: Missing OpenCode model/provider configuration",
          "yellow",
        );
